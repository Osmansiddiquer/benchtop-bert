...
A second contribution is methodological: a pattern for pulling \textbf{continuous
internal-health telemetry off a live training run at approximately zero marginal GPU cost}
(\S\ref{sec:zerocost})---atomic checkpoints so the run is readable from outside, scoring on the
idle device (8.9--22.2\,s on CPU against 13.2--16.6 minutes of exclusive GPU for a downstream
evaluation), and streaming the already-free per-layer signals straight into the training log.
...
post-LN transformers. Section~\ref{sec:depth} reports the depth result, and
Section~\ref{sec:diag} the diagnostics---both the zero-cost telemetry pattern that made the
project observable at all (\S\ref{sec:zerocost}) and the ways the cheap metrics misled.
Sections~\ref{sec:anneal}--\ref{sec:width} report the remaining results. Section~\ref{sec:eng} collects engineering findings that cost real time.
...
not \textbf{validity}. Cheap telemetry is not free of interpretation risk, and three of these
